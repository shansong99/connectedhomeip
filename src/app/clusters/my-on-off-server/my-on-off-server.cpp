...
#endif
        {
            MyOnOffServer::Instance().scheduleTimerCallbackMs(sceneEventControl(endpoint), timeMs);
        }
...
private:
    ...
    EmberEventControl * sceneEventControl(EndpointId endpoint)
    {
        EmberEventControl * controller =
            MyOnOffServer::Instance().getEventControl(endpoint, Span<EmberEventControl>(sceneHandlerEventControls));
...
static void sceneOnOffCallback(EndpointId endpoint)
{
    ...
    MyOnOffServer::Instance().setOnOffValue(endpoint, command, false);
...
static MyOnOffEffect * firstEffect = nullptr;
MyOnOffServer MyOnOffServer::instance;
...
static MyOnOffEffect * inst(EndpointId endpoint);

/**********************************************************
 * Matter timer scheduling glue logic
 *********************************************************/

void MyOnOffServer::timerCallback(System::Layer *, void * callbackContext)
{
    auto control = static_cast<EmberEventControl *>(callbackContext);
    (control->callback)(control->endpoint);
}

void MyOnOffServer::scheduleTimerCallbackMs(EmberEventControl * control, uint32_t delayMs)
{
...
void MyOnOffServer::cancelEndpointTimerCallback(EmberEventControl * control)
{
    DeviceLayer::SystemLayer().CancelTimer(timerCallback, control);
}

void MyOnOffServer::cancelEndpointTimerCallback(EndpointId endpoint)
{
    auto control = MyOnOffServer::getEventControl(endpoint, Span<EmberEventControl>(gEventControls));
    if (control)
    {
...
void MatterMyOnOffClusterServerShutdownCallback(EndpointId endpoint)
{
    ChipLogProgress(Zcl, "Shuting down on/off server cluster on endpoint %d", endpoint);
    MyOnOffServer::Instance().cancelEndpointTimerCallback(endpoint);
}
...
chip::scenes::SceneHandler * MyOnOffServer::GetSceneHandler()
{
...
bool MyOnOffServer::HasFeature(chip::EndpointId endpoint, Feature feature)
{
    bool success;
    uint32_t featureMap;
...
EmberAfStatus MyOnOffServer::getOnOffValue(chip::EndpointId endpoint, bool * currentOnOffValue)
{
    // read current on/off value
 ...
 */
EmberAfStatus MyOnOffServer::setOnOffValue(chip::EndpointId endpoint, chip::CommandId command, bool initiatedByLevelChange)
{
...
void MyOnOffServer::initOnOffServer(chip::EndpointId endpoint)
{
    ...
    if (SupportsLightingApplications(endpoint) && areStartUpOnOffServerAttributesNonVolatile(endpoint))
    {
        ...
        app::Clusters::Scenes::ScenesServer::Instance().RegisterSceneHandler(endpoint, MyOnOffServer::Instance().GetSceneHandler());
#endif
    ...
    emberAfPluginMyOnOffClusterServerPostInitCallback(endpoint);
}
 ...
 */
EmberAfStatus MyOnOffServer::getOnOffValueForStartUp(chip::EndpointId endpoint, bool & onOffValueForStartUp)
{
    app::DataModel::Nullable<MyOnOff::OnOffStartUpOnOff> startUpOnOff;
    ...
    if (status == EMBER_ZCL_STATUS_SUCCESS)
    {
        ...
        if (status == EMBER_ZCL_STATUS_SUCCESS)
        {
            if (!startUpOnOff.IsNull())
            {
                ...
                {
                case MyOnOff::OnOffStartUpOnOff::kOff:
                    updatedOnOff = false; // Off
                    break;
                case MyOnOff::OnOffStartUpOnOff::kOn:
                    updatedOnOff = true; // On
                    break;
                case MyOnOff::OnOffStartUpOnOff::kTogglePreviousOnOff:
...
bool MyOnOffServer::offCommand(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath)
{
...
bool MyOnOffServer::onCommand(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath)
{
...
bool MyOnOffServer::toggleCommand(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath)
{
...
bool MyOnOffServer::offWithEffectCommand(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,
                                         const Commands::OffWithEffect::DecodableType & commandData)
{
    ...
    if (SupportsLightingApplications(endpoint))
    {
        ...
        MyOnOff::Attributes::GlobalSceneControl::Get(endpoint, &globalSceneControl);

        bool isOnBeforeCommand = false;
        MyOnOff::Attributes::OnOff::Get(endpoint, &isOnBeforeCommand);
...
#endif // EMBER_AF_PLUGIN_SCENES
            MyOnOff::Attributes::GlobalSceneControl::Set(endpoint, false);
        }

        // Only apply effect if OnOff is on
        if (isOnBeforeCommand)
        {
            MyOnOffEffect * effect = inst(endpoint);
...
bool MyOnOffServer::OnWithRecallGlobalSceneCommand(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath)
{
    chip::EndpointId endpoint = commandPath.mEndpointId;
    ...
    MyOnOff::Attributes::GlobalSceneControl::Set(endpoint, true);
...
uint32_t MyOnOffServer::calculateNextWaitTimeMS()
{
...
bool MyOnOffServer::OnWithTimedOffCommand(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,
                                          const Commands::OnWithTimedOff::DecodableType & commandData)
{
    ...
    MyOnOff::Attributes::OnOff::Get(endpoint, &isOn);
    ...
    MyOnOff::Attributes::OffWaitTime::Get(endpoint, &currentOffWaitTime);
    MyOnOff::Attributes::OnTime::Get(endpoint, &currentOnTime);

    if (currentOffWaitTime > 0 && !isOn)
    {
        uint16_t newOffWaitTime = currentOffWaitTime < offWaitTime ? currentOffWaitTime : offWaitTime;
        MyOnOff::Attributes::OffWaitTime::Set(endpoint, newOffWaitTime);
    ...
    else
    {
        ...
        MyOnOff::Attributes::OnTime::Set(endpoint, newOnTime);

        MyOnOff::Attributes::OffWaitTime::Set(endpoint, offWaitTime);
        setOnOffValue(endpoint, Commands::On::Id, false);
 ...
 */
void MyOnOffServer::updateOnOffTimeCommand(chip::EndpointId endpoint)
{
    ...
    if (isOn) // OnOff On case
    {
        ...
        MyOnOff::Attributes::OnTime::Get(endpoint, &onTime);
        ChipLogDetail(Zcl, "Timer callback - On Time:  %d", onTime);

        if (onTime > 0)
        {
            onTime--;
            MyOnOff::Attributes::OnTime::Set(endpoint, onTime);
        }
        ...
        if (onTime == 0)
        {
            ...
            MyOnOff::Attributes::OffWaitTime::Set(endpoint, 0);
    ...
    else // OnOff Off Case
    {
        ...
        MyOnOff::Attributes::OffWaitTime::Get(endpoint, &offWaitTime);
...
#ifndef IGNORE_ON_OFF_CLUSTER_START_UP_ON_OFF
bool MyOnOffServer::areStartUpOnOffServerAttributesNonVolatile(EndpointId endpoint)
{
    return !emberAfIsKnownVolatileAttribute(endpoint, MyOnOff::Id, Attributes::OnOff::Id) &&
        !emberAfIsKnownVolatileAttribute(endpoint, MyOnOff::Id, Attributes::StartUpOnOff::Id);
}
#endif // IGNORE_ON_OFF_CLUSTER_START_UP_ON_OFF
 ...
 * @return EmberEventControl* configured event control
 */
EmberEventControl * MyOnOffServer::getEventControl(EndpointId endpoint, const Span<EmberEventControl> & eventControlArray)
{
    uint16_t index = emberAfGetClusterServerEndpointIndex(endpoint, MyOnOff::Id, EMBER_AF_ON_OFF_CLUSTER_SERVER_ENDPOINT_COUNT);
 ...
 * @return EmberEventControl* configured event control
 */
EmberEventControl * MyOnOffServer::configureEventControl(EndpointId endpoint)
{
    ...
    controller->endpoint = endpoint;
    controller->callback = &myOnOffWaitTimeOffEventHandler;

    return controller;
}

/**********************************************************
 * MyOnOffEffect Implementation
 *********************************************************/

static MyOnOffEffect * inst(EndpointId endpoint)
{
    MyOnOffEffect * current = firstEffect;
...
static inline void reg(MyOnOffEffect * inst)
{
    inst->setNext(firstEffect);
    firstEffect = inst;
}

static inline void unreg(MyOnOffEffect * inst)
{
    if (firstEffect == inst)
    {
    ...
    {
        MyOnOffEffect * previous = firstEffect;
        MyOnOffEffect * current  = firstEffect->next();
...
MyOnOffEffect::MyOnOffEffect(chip::EndpointId endpoint, OffWithEffectTriggerCommand offWithEffectTrigger,
...
bool emberAfMyOnOffClusterOffCallback(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,
                                      const Commands::Off::DecodableType & commandData)
{
    return MyOnOffServer::Instance().offCommand(commandObj, commandPath);
}

bool emberAfMyOnOffClusterOnCallback(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,
                                     const Commands::On::DecodableType & commandData)
{
    return MyOnOffServer::Instance().onCommand(commandObj, commandPath);
}

bool emberAfMyOnOffClusterToggleCallback(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,
                                         const Commands::Toggle::DecodableType & commandData)
{
    return MyOnOffServer::Instance().toggleCommand(commandObj, commandPath);
}

bool emberAfMyOnOffClusterOffWithEffectCallback(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,
                                                const Commands::OffWithEffect::DecodableType & commandData)
{
    return MyOnOffServer::Instance().offWithEffectCommand(commandObj, commandPath, commandData);
}

bool emberAfMyOnOffClusterOnWithRecallGlobalSceneCallback(app::CommandHandler * commandObj,
                                                          const app::ConcreteCommandPath & commandPath,
                                                          const Commands::OnWithRecallGlobalScene::DecodableType & commandData)
{
    return MyOnOffServer::Instance().OnWithRecallGlobalSceneCommand(commandObj, commandPath);
}

bool emberAfMyOnOffClusterOnWithTimedOffCallback(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath,
                                                 const Commands::OnWithTimedOff::DecodableType & commandData)
{
    return MyOnOffServer::Instance().OnWithTimedOffCommand(commandObj, commandPath, commandData);
}

void emberAfMyOnOffClusterServerInitCallback(chip::EndpointId endpoint)
{
    MyOnOffServer::Instance().initOnOffServer(endpoint);
}

void myOnOffWaitTimeOffEventHandler(chip::EndpointId endpoint)
{
    MyOnOffServer::Instance().updateOnOffTimeCommand(endpoint);
}

void emberAfPluginMyOnOffClusterServerPostInitCallback(EndpointId endpoint) {}

void MatterMyOnOffPluginServerInitCallback() {}
